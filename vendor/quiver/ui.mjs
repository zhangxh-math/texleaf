import TexLeafKaTeX from "./KaTeX/katex.mjs";
...
class UI {
    constructor(element) {
        ...
        Object.assign(this.settings.data, { "quiver.renderer": "katex", "quiver.theme": CONSTANTS.THEMES.LIGHT, "quiver.autosave": false });
    ...
    initialise() {
        ...
        const version_previous_use = CONSTANTS.VERSION;
            ...
            if (options.style.name.startsWith("corner")) {
                button.set_style({
                    "background-image": ["", "un"].map((prefix) => {
                        return `url("${TEXLEAF_QUIVER_ICONS[`${options.style.name.endsWith("inverse") ? "var-" : ""}pullback-${prefix}checked.svg`]}")`;
                    }).join(", ")
                });
                return button;
            }
                ...
                KaTeX.then((katex) => {
                    katex.render(
                        ...
                        label.element,
                        {
                            ...
                            trust: false, // Offline labels cannot enable trusted HTML or external resources.
                        },
                    );
    ...
    initialise(ui) {
        ...
        const add_action = (label, name, combinations, action, element = this.element) => {
            ...
            const button = new DOM.Element("button", { class: "action", "data-name": name })
                .add(new DOM.Element("span", { class: "symbol" }).add(
                    new DOM.Element("img", { src: TEXLEAF_QUIVER_ICONS[`${name}.svg`] })
                ))
        ...
        const update_autosave_action = () => {
            ...
            element.query_selector(".symbol img").set_attributes({
                src: TEXLEAF_QUIVER_ICONS[`autosave-${autosave ? "off" : "on"}.svg`],
            });
        };
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    KaTeX = Promise.resolve({default: TexLeafKaTeX}).then((module) => {
        ...
        queueMicrotask(() => window.dispatchEvent(new CustomEvent("texleaf-quiver-ready", { detail: ui })));
