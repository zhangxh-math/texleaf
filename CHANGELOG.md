...
- 活动行和展开导言区的背景不再遮住文字选区，选区仍使用当前 VS Code 主题颜色。
- 源码包装按钮随编辑与滚动更新定位范围，避免复用旧位置。
- 打开的 quiver 交换图草稿会阻止切换源码；先应用或取消后即可切换；连续标准/增强切换时保留草稿。

## [1.2.1] - 2026-09-09

### Added

- 回迁 TexLeaf-Z 截至 `f4d8813` 的可视化改进，交换图入口统一为内置中文 quiver 1.7.0：支持节点与箭头拖动、完整箭头样式、弯曲、标签、颜色和放大编辑；应用修改沿用文档版本校验与一次撤销，未修改应用或取消保留原文。
- quiver 原生状态随源码保存，外部修改后重新解析 TikZ；不支持的导入选项显示提示，修改后转换需明确勾选，仍提供原始 `tikzcd` 源码入口。主文档按需补充 quiver 宏包，子文件需在主文档加载。
- 实际嵌入 q.uiver.app / varkor/quiver 1.7.0 源码（上游提交 `2f289ecbae9b7e5a473e04b924750c538ed5c4cf`，MIT，Copyright (c) 2018 varkor），包含本地汉化与集成补丁；quiver、KaTeX 0.18.1、字体、图标和原许可证本地打包，增强预览随附 quiver.sty。
- Beamer 支持局部分组的字号、粗体、斜体、命名颜色和可确定的单参数包装宏；columns/column 纵向显示正文并保留源码入口，兼容文本与数学 alert。
- 新增 `texleaf.visualEditor.previewZoomPercent`：增强图形默认 150%，可设置及调节 50–400%；设置立即生效，重置回到设置值，缩放不重新编译、不改变源码或 PDF。

### Fixed

- 修复公式内中文 IME 候选替换、删除和取消后无法继续输入或退格，以及缺少 composition 结束通知时的恢复。
- 修复展开表格源码与混合文字单元格的 Math Preview、预览起点对齐、单元格空格误触源码展开、工具栏/右键插入无效，以及多行片段缩进未同步到文件。
- 修复 Beamer 样式作用域、重定义、嵌套字号与显式换行叠加；公式 alert 使用兼容数学显示。
- 改进 LaTeX Workshop 桥接的 Beamer SyncTeX：普通帧使用可核验原行或本帧结束记录的首个命中，避免跳到上一帧；精确/fragile/direct 帧保留原行，反向帧末命中返回帧头，精确正文仍定位原处。统一可视化 LF 与文档 LF/CRLF 坐标转换，避免换行偏移累积。
- 图形卡片、quiver 画布、图注、按钮和图形引用悬浮统一使用不透明浅色纸面；移除图形黑白替换与位图反色，保留原图颜色、透明度、渐变及空实心节点，主题切换无需重编译。
- README、配置、开发指南、Wiki 与第三方通知同步到本版，旧交换图 GIF 明确标记为历史交互演示。

### Compatibility

- 延续 1.2.0 的标准/增强模式、本机图形依赖、进度/取消/缓存、文首/摘要与空命令源码入口、脚注/目录/附录/计数器、标题和文献公式，以及局部源码范围、异步过期结果与缓存失效修复；原历史记录保留于下方。
- 编译、PDF、SyncTeX 和编译诊断仍使用 LaTeX Workshop 兼容桥，包括其已加载的运行时模块与公开命令回退；未迁入 TexLeaf-Z 首页、账号、Git、全文搜索或独立编译/PDF 工作流。TeXLeaf 主体保留 GPL-3.0-only、NOTICE 署名与用户文档输出例外。

## [1.2.0] - 2026-09-09

### Added

- 可视化编辑器新增“标准可视化 / 增强可视化”切换。标准模式使用 MathJax；增强模式使用本机 TeX 排版复杂公式、TikZ、交换图和组合图片，支持进度、取消与有容量上限的图形缓存。
- 支持正文局部宏、脚注、定理样式、图题，以及更完整的标题、目录和附录编号。可验证的 LaTeX Workshop 成功编译结果可提供引用编号；过期或来源不明的产物不覆盖当前源码推断。

### Fixed

- 回迁 TexLeaf-Z 0.10.2 的可视化修复，包括标题中的引用与公式、局部源码范围、无法预览时的提示、连续环境布局、异步渲染过期结果和缓存失效。
- README 图标与八段 GIF 使用公开绝对地址，修复 VSIX 扩展详情中的相对路径显示问题。
- 保留 TeXLeaf 的原生源码入口、补全、Undo/Redo、导航和 LaTeX Workshop 编译、PDF、SyncTeX 工作流。

## [1.1.1] - 2026-09-08

### Fixed

- 防止可视化标签延迟初始化时抢走原生源码焦点，并使未保存的 `.bbl` 编辑及时刷新文献预览。

- 完整回迁 TexLeaf-Z 的可视化编辑修复：文首标题与作者、目录编号与附录、公式宏与复杂环境、列表/定理/表格/图片布局、引用预览/补全/跳转、源码高亮和括号配色、滚动后的公式补绘，以及源码范围与编辑交互。
- 支持安全的 PDF/SVG 本地插图预览与生成的 `.bbl` 文献列表；按论文实际声明与路径选择参考文献，更新后自动失效缓存。
- 修复原生 bibliography 与可视化编辑器之间的返回/前进，补齐导航快捷键到宿主的消息处理。
- 保留 TeXLeaf 的原生源码入口、VS Code Problems 和 LaTeX Workshop 编译/PDF/SyncTeX 兼容路径。
- 复杂 TikZ、原生 TeX 垂直盒子、杨图组合 figure 和部分附录布局仍有预览限制；保留源码与提示，完整排版以 TeX 编译 PDF 为准。

## [1.1.0] - 2026-09-03

### Fixed

- 修复 Windows CRLF 文档在可视化编辑器中的坐标随行号逐渐漂移的问题。Extension Host、CodeMirror 协议、项目扫描、TextMate/括号着色、公式与结构组件、补全、AI 诊断、引用导航和 SyncTeX 现在统一使用每个逻辑换行只占一个字符的 LF 坐标；只有写回 VS Code `TextDocument` 时才恢复原文件的 CRLF，因此现有文件不会被悄悄改换行格式。
- 修复上述坐标漂移引起的连锁表现，包括长文档中源码配色错位、快速滚动停止后可见公式或结构仍不渲染、标题/正文/原始命令残片互相重叠，以及点击、Problems 条目、跨文件标签和补全替换落到错误字符或错误行。

### Verification

- 新增 LF、CRLF、混合换行、不可表示的 CR/LF 中间偏移、越界偏移与多文件 CRLF 项目范围回归；项目扫描继续优先使用未保存的打开缓冲区，并验证 include 与 label 的半开区间能准确切回原始命令。
- 本版只从 TexLeaf-Z 回迁与桥接版 TexLeaf 共用的问题修复；未引入 TexLeaf-Z 的独立编译、项目管理、Git、托管平台或同步功能，LaTeX 构建仍由 LaTeX Workshop 提供。
