...
这是一份 223 条工厂默认规则的入门速查，不是机器生成的完整清单。要查看、搜索或修改当前实际规则，请运行 `TeXLeaf: 管理 Snippet 与模板`；快速插入可用 `TeXLeaf: 搜索并插入片段` 或侧栏。高级 JSONC 后端仍保存完整库，但不是日常管理入口。
...
四个长模板不计入上面的 223 条 Snippet 规则，而是保存在 Profile 的插件内部模板库：
...
| `bino` | `\binom{@0}{@1}@2` | `mA` | 依次填写二项式系数的上下参数，再离开整个命令；外层普通括号会像分式一样自动放大。 |
| `dd` | `\dd ` | `mA` | 插入与工厂模板配套的直立微分符号。 |
| `part` | `\partial` | `mA` | 插入偏导符号。 |
| `res` | `\operatorname{Res}` | `mA` | 插入留数算子。 |
| `sst` | `\substack{@0}` | `mA` | 创建多行上下标结构。 |

自动分数是另一项独立功能：启用 `texleaf.autoFraction` 后，在数学区域输入单个 `/` 时，TeXLeaf 先保留斜杠；输入首个有效分母字符后，才把左侧局部表达式取为分子。例如 `1/` 会保持原样，接着输入 `2` 才得到 `\frac{1}{2}`。`<` 与 `>` 是默认分子边界，因此 `<1/2` 得到 `<\frac{1}{2}`，不会把关系符塞进分子。自动片段的匹配顺序更靠前，因此输入第二个 `/` 仍会触发明确的 `//` 片段，不会被自动分数截断。若不希望自动分析，关闭该设置即可继续单独使用 `//`。
...
正则匹配不会从 TeX 控制词中间开始，因此 `x0` 仍得到 `x_{0}`，而 `\leq0`、`\cdots0` 和 `\alpha2` 会保持原样。若自定义正则确实要匹配整个命令，应从反斜杠开始编写 trigger。
...
- `Tab` 在活动 tabstop 处理完成后，只越过当前单元格内真正包围光标、与最内层 opener 配对的闭合符；没有局部 Tabout 目标时才插入下一列的 ` & `；
