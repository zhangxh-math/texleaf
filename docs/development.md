...
- 分别输入 `\axm`、`\dfn`、`\lem`、`\prp`、`\thm`、`\cor`、`\clm`、`\asm`、`\exm`、`\exr`、`\cnj`、`\hyp`、`\rmk`，确认全部在文本模式自动展开为对应环境；在原生 Suggest 中确认非精确候选保留全部适用片段中的全局最长非零前缀组，并在该组内保持 VS Code 原生排序；所有已完整输入的 literal trigger 即使不属于最长组也必须额外保留。覆盖 regex shadow 和重复 ID：只有 runtime 唯一选中的 exact literal 获得 Keyword 类型、preselect 与 exact sort，其他完整 literal 仍是普通 Snippet 候选。特别输入 `ss`，确认保留匹配两个字符的 `SS2`、`SSE`、`SSP`、`SSS`，不混入只匹配末尾单个 `s` 的 `sum`、`sim`、`sub`、`sup`；退回单个 `s` 和输入完整 trigger 时仍正常。再模拟模板、定理片段或 `dm` 自动展开未触发但 Suggest 已出现的情形，按 `Tab` 应展开精确 TeXLeaf 片段，而不是接受普通单词。
...
- 在数学区域创建仍有下一 tabstop 的活动 Snippet Session，并让 Suggest 显示竞争候选；按 Tab 应关闭 Suggest 并前往下一占位符，不得接受候选或执行 Tabout。再分别在 Inline Suggest、Rename 输入框和配置的 Matrix/Align 上下文中重复按 Tab：前两者保持各自原生优先级；Matrix/Align 中若当前单元格有真实右侧闭合符则先 Tabout，没有目标时才回退插列。
...
- 展开 `align`、`matrix` 或 `pmat`，验证 `Tab` 先处理活动 tabstop，再处理当前单元格内的 Tabout，最后才执行列操作；`Enter` 创建新行，`Shift+Enter` 退出环境。加入精确回归：让普通 `(` 与 `)` 分处 `&`、`\\`、`\cr`、`\crcr`、`\tabularnewline` 两侧后输入 `sum`，不得生成跨单元格/跨行 `\left...\right`，光标应停在局部 `\sum` 之后；在 `align*` 的 `\frac{1}{n^{2}|}` 中第一次 `Tab` 必须越过分母 `}` 且不插入 ` & `，下一次没有局部闭合符时才插列。转义 `\&` 和注释中的边界同形文本不得误触发阻断。
