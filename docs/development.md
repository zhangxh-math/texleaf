...
以下脚本启动隔离的 VS Code Profile，只编辑测试副本。需要本机 `code`、Node.js、TeX 引擎及对应宏包；串行运行，并按机器资源限制进程总内存。

```bash
node test/run-visual-compatibility-qa.cjs title-citation --full --source-audit --boundary-audit
node test/run-visual-compatibility-qa.cjs performance --full --exercise-cache
TEXLEAF_WORKSHOP_PATH="/path/to/installed/latex-workshop" node test/run-visual-compatibility-qa.cjs compiled --full
```
